...
export function ShadowdarkRenderer({ data }: SystemRendererProps<ShadowdarkData>) {
  ...
  return (
    <div
      className="relative rounded-none font-sans text-black bg-white max-w-2xs mx-auto p-4 space-y-2"
    >
      {/* Monster Name */}
      <h1
        className="text-center text-xl font-bold p-1 bg-black text-white uppercase"
      >
        {data.name}
      </h1>
      ...
      {/* Description (italic) */}
      {data.description && (
        <p className="text-left italic text-sm leading-snug">
          {data.description}
        </p>
      )}
        ...
        <p>
          ...
          {ABILITY_KEYS.map((key, idx) => (
            <React.Fragment key={key}>
              <span className="font-bold">{ABILITY_LABELS[key]}</span> {formatModifier(getAbilityScore(key))}
